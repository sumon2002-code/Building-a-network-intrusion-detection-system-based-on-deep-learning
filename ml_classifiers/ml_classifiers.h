...
void classify_connections() {
    ...
    outputFile.open("/home/hieupham/Desktop/logs/logs.txt", std::ios_base::trunc);
    
    for (int i = 0; i < t_connections.id.size(); i++) {
        // std::cout << "[-] " << t_connections.id[i] << std::endl;
        // t_connections.connections[i].print_feature_vector(t_connections.features[i]);
        std::time_t current_time = std::time(nullptr);
        char time_buffer[100];
        std::strftime(time_buffer, sizeof(time_buffer), "%Y-%m-%d_%H:%M:%S", std::localtime(&current_time));

        outputFile << std::fixed << std::setprecision(9);

        outputFile << time_buffer << " ";

        outputFile << t_connections.id[i] << " ";
        for (int j = 0; j < 78; j++) {
    ...
    outputFile.close();
    int ret_code = system("python3 /home/hieupham/Desktop/FlaskIDS/ids.py");
    if (ret_code != 0) {
    std::cerr << "Error running Python script, return code: " << ret_code << std::endl;
    return;
}
